...
export const TilingEditor = ({ rooms, config, setConfig }: TilingEditorProps) => {
  ...
  return (
    <div className="flex flex-1 overflow-hidden bg-zinc-950">
      <div className="relative flex flex-1 flex-col border-r border-zinc-900">
        <TilingCanvas
          svgRef={svgRef}
          rooms={rooms}
          tiles={tiles}
        ...
        <div className="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 items-center gap-5 rounded-2xl border border-zinc-800 bg-zinc-900/90 px-5 py-3 shadow-2xl backdrop-blur-md">
          {/* Dimensions toggle */}
          <button
            type="button"
            ...
            className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider transition-all ${
              showDimensions
                ...
                : 'border border-zinc-700 bg-zinc-800 text-zinc-500 hover:border-zinc-500 disabled:opacity-30'
            }`}
          >
            <Ruler size={12} /> Côtes
          </button>
          ...
          <div className="flex items-center gap-2.5">
            <span className="w-14 text-right text-[10px] font-bold uppercase tracking-wider text-zinc-500">Angle</span>
            <input
              type="range" min="0" max="90" step="1"
              value={config.angle}
              onChange={(e) => setConfig({ ...config, angle: parseInt(e.target.value, 10) })}
              className="h-1.5 w-24 cursor-pointer appearance-none rounded-lg bg-zinc-700 accent-orange-500"
            />
            <span className="w-10 font-mono text-xs font-bold text-orange-400">{config.angle}°</span>
          </div>
          <div className="h-5 w-px bg-zinc-700" />
          <div className="flex items-center gap-2.5">
            <span className="w-14 text-right text-[10px] font-bold uppercase tracking-wider text-zinc-500">Décal. X</span>
            <input
              ...
              className="h-1.5 w-20 cursor-pointer appearance-none rounded-lg bg-zinc-700 accent-orange-500"
            />
          </div>
          <div className="flex items-center gap-2.5">
            <span className="w-14 text-right text-[10px] font-bold uppercase tracking-wider text-zinc-500">Décal. Y</span>
            <input
      ...
      <aside className="z-20 flex w-80 flex-col overflow-y-auto bg-zinc-900 shadow-2xl">
